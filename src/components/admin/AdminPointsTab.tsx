...
import { CheckCircle, XCircle, Clock, Loader2, Search, ChevronLeft, ChevronRight } from 'lucide-react';
                ...
                {tx.status === 'pending' && (
                  <div className="flex gap-2 mt-3 pt-3 border-t border-border">
                    <button
                      ...
                      className="flex-1 flex items-center justify-center gap-2 py-2 rounded-lg bg-green-500 text-white text-sm font-medium hover:bg-green-600 disabled:opacity-50"
                    >
                      {processingId === tx.id ? (
                    ...
                    <button
                      ...
                      disabled={processingId === tx.id}
                      className="flex-1 flex items-center justify-center gap-2 py-2 rounded-lg bg-red-500 text-white text-sm font-medium hover:bg-red-600 disabled:opacity-50"
                    >
                      {processingId === tx.id ? (
                        <Loader2 className="h-4 w-4 animate-spin" />
                      ) : (
        ...
        <AlertDialogContent>
          <AlertDialogHeader>
            ...
            <AlertDialogDescription>
              {confirmDialog.transaction && (
                <>
                  <strong>{confirmDialog.transaction.user_nickname}</strong>님의{' '}
                  <strong>{confirmDialog.transaction.amount}P</strong>를{' '}
                  {confirmDialog.type === 'confirm' ? '확정' : '취소'}하시겠습니까?
                </>
              )}
            </AlertDialogDescription>
          </AlertDialogHeader>
          <AlertDialogFooter>
            ...
            <AlertDialogAction
              ...
              className={confirmDialog.type === 'confirm' ? 'bg-green-500 hover:bg-green-600' : 'bg-red-500 hover:bg-red-600'}
            >
      ...
      <AlertDialog open={batchDialog.open} onOpenChange={(open) => setBatchDialog(prev => ({ ...prev, open }))}>
        <AlertDialogContent>
          <AlertDialogHeader>
            ...
            <AlertDialogDescription>
              선택한 <strong>{selectedIds.size}건</strong>의 포인트를{' '}
              {batchDialog.type === 'confirm' ? '확정' : '취소'}하시겠습니까?
            </AlertDialogDescription>
          </AlertDialogHeader>
          <AlertDialogFooter>
            <AlertDialogCancel>취소</AlertDialogCancel>
            <AlertDialogAction
              ...
              className={batchDialog.type === 'confirm' ? 'bg-green-500 hover:bg-green-600' : 'bg-red-500 hover:bg-red-600'}
            >
              {batchDialog.type === 'confirm' ? '일괄 확정' : '일괄 취소'}
            </AlertDialogAction>
          </AlertDialogFooter>
