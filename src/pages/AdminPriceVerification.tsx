...
import { ArrowLeft, Check, X, Image, User, Store, Package, Calendar, Coins } from 'lucide-react';
  ...
  const fetchPendingItems = async () => {
    ...
    try {
      ...
      for (const tx of transactions || []) {
        ...
        // Get price history if reference exists
        if (tx.reference_id) {
    ...
    <div className="min-h-screen bg-background">
      ...
      <header className="sticky top-0 z-40 bg-background border-b">
        <div className="flex items-center gap-3 px-4 py-3">
          ...
          <div>
            <h1 className="text-lg font-bold">가격 등록 검증</h1>
            <p className="text-xs text-muted-foreground">
              대기 중: {pendingItems.length}건
            </p>
          </div>
        </div>
      </header>
...
export default AdminPriceVerification;
